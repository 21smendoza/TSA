...
def outputMessageLBTM(text,color,textWidth,textHeight,i):
    ...
    textRect.center = (textWidth,textHeight)
    screen.blit(textSurface,textRect)
    return

def textShifterAutomaticTKBerg(text,color,coord,i):
    """takes long text sequences and resizes the phrase to fit inside the window"""
...
if True:
    ...
    select = "Pick three players to use. High attack makes the opposing receiver more likely to drop the ball. High defense makes the player less likely to drop the ball."
    ...
    selecting = "Pick a card, and pick the tile that you would like to place them in. After placement, you will be unable to switch positions."
    ### server = font_small.render("Pick a character to serve the ball.", True, black)
    server = "Pick a character to serve the ball. Keeping in mind the boards are not mirrored, the ball will always go towards the square counterpart to the hitter."
    flying = "Look out! The opponent is about to hit the ball back! Click on a player to maneuver into a new position. However, you can only move twice at most, so choose wisely."
...
play_button, quit_button = Menu_button(width / 2, height / 3.5), Menu_button(width / 2, height / 2.5)
...
while menu:
    ...
    outputMessageLBTM(title, black, screen.get_width() / 2, height / 10, 1)
    ### screen.blit(play, (width / 2.05, height / 3.45))
    outputMessageLBTM("Play", black, play_button.rect.x + play_button.image.get_width()/2, play_button.rect.y + play_button.image.get_height()/2, 0)
    ### screen.blit(end_game, (width / 2.05, height / 2.38))
    outputMessageLBTM("Quit", black, quit_button.rect.x + quit_button.image.get_width()/2, quit_button.rect.y + quit_button.image.get_height()/2, 0)
...
proceed_button = Menu_button(width / 2, height / 2)
...
while selection:
    ...
    textShifterAutomaticTKBerg(select, black, (10, 10), 0)
    #if the 3 cards are selected, then the option to continue becomes available
    if len(selected) == 3:
        ### screen.blit(proceed, (width / 2.3, height / 1.96))
        outputMessageLBTM("Continue", black, proceed_button.rect.x + proceed_button.image.get_width()/2, proceed_button.rect.y + proceed_button.image.get_height()/2, 0)
...
while selection_2:
    ...
    textShifterAutomaticTKBerg(selecting, black, (10, 10),0)
    cursor.update()
    cursor_group.draw(screen)
...
while run:
    ...
    while serving:
        ...
        textShifterAutomaticTKBerg(server, black, (10, 10), 0)
